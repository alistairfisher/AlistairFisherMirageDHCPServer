...
module Make (Console:V1_LWT.CONSOLE)
  ...
  (*TODO: use a new type for client ID to differentiate explicit ID from hardware address, allowing different hardware types.*)  
  ...
  (*TODO: supplement subnet type with a server IP that should be used for all clients on this subnet*)  
  type subnet = {
    subnet: Ipaddr.V4.t;
    netmask: Ipaddr.V4.t;
  ...
  let output_broadcast ?nak:(n=false) t ~xid ~ciaddr ~yiaddr ~siaddr ~giaddr ~secs ~chaddr ~flags ~options =
    ...
    let dest_ip_address =
      if(giaddr <> Ipaddr.V4.unspecified) then giaddr (*giaddr set: forward onto the correct BOOTP relay*) (*see RFC 2131 page 22 for more info*)
  ...
  let input t ~src:_ ~dst:_ ~src_port:_ buf = (*lots of duplication with client, need to combine into one unit*)
    ...
    match packet.op with
      |`Discover -> (*should probe address via ICMP here, and ensure that it's actually free, and try a new one if not*)
        ...
        (*send DHCP Offer*)
        output_broadcast t ~xid:xid ~ciaddr:ciaddr ~yiaddr:reserved_ip_address ~siaddr:serverIP ~giaddr:giaddr ~secs:secs ~chaddr:chaddr ~flags:flags ~options:options;
      |`Request ->
        ...
        (match (find packet(function `Server_identifier id ->Some id |_ -> None)) with
          |Some id -> (*This is a true request, client has no IP address*)
            ...
            if ((List.mem server_identifier (t.serverIPs)) && ((List.assoc client_identifier !(client_subnet.reserved_addresses)).xid=xid)) then ( (*the client is requesting the IP address, this is not a renewal. Need error handling*)
              ...
              output_broadcast t ~xid:xid ~ciaddr:ciaddr ~yiaddr:ip_address ~siaddr:serverIP ~giaddr:giaddr ~secs:secs ~chaddr:chaddr ~flags:flags ~options:options;
            )
          ...
          |None -> (*this is a renewal, rebinding or init_reboot. TODO: check whether requested IP is on the correct subnet*)
            if (ciaddr = Ipaddr.V4.unspecified) then (*client in init-reboot*)
              ...
              if (List.mem requested_IP !(client_subnet.available_addresses)) then (*address is available, lease to client*)
                ...
                output_broadcast t ~xid:xid ~nak:true ~ciaddr:ciaddr ~yiaddr:(Ipaddr.V4.unspecified) ~siaddr:(Ipaddr.V4.unspecified) ~giaddr:giaddr ~secs:secs ~chaddr:chaddr ~flags:flags ~options:options;
